#****************************************************************************
#* setup.py for zuspec-langserver
#****************************************************************************
import os
import sys
import subprocess
...
libjson_rpc_dir = os.path.dirname(os.path.abspath(__file__))

if os.path.isdir(os.path.join(libjson_rpc_dir, "packages")):
    print("Packages are inside this directory")
    packages_dir = os.path.join(libjson_rpc_dir, "packages")
else:
    parent = os.path.dirname(libjson_rpc_dir)
    
    if os.path.isdir(os.path.join(parent, "libjson-rpc")):
        print("libjson-rpc is a peer")
        packages_dir = parent
    else:
...
#            )
    ...
    def copy_extensions_to_source(self):
        ...
        copy_file(
            os.path.join(cwd, "build", "src", "libjson-rpc.so"),
            os.path.join(package_dir, "libjson-rpc.so"))
...
ext = Extension("libjson_rpc.core",
            extra_compile_args=extra_compile_args,
            sources=[
                os.path.join(libjson_rpc_dir, 'python', "core.pyx"), 
            ],
            language="c++",
            include_dirs=[
                os.path.join(libjson_rpc_dir, 'python'),
                os.path.join(libjson_rpc_dir, 'src', 'include'),
...
setup(
  ...
  url = "https://github.com/mballance-utils/libjson_rpc",
  install_requires=[
    'debug-mgr'
  ],
  setup_requires=[
